...
class _LatencyTracker:
    ...
    def print_report(self):
        ...
        print(f"  LLM steps:           {s['llm_steps']} (total {s['llm_time_sec']:.3f}s)")
        print(f"  Tool calls:          {s['tool_calls']} (total {s['tool_time_sec']:.3f}s)")
...
class Agent:
    ...
    async def _run(self):
        ...
        events = runner.run_async(session_id=session.id, user_id="local_admin", new_message=content)
        ...
        async for event in events:
            if event.is_final_response():
                ...
                # Always show the final agent message
                ...
                for part in event.content.parts:
                    if getattr(part, "function_call", None):
                        ...
                        if latency:
                            ...
                            if self.verbose:
                                print(f"[{step_sec:.3f}s LLM→tool] [Action]: {name}({args})")
                        else:
                            if self.verbose:
                                print(f"[{round(now - _last, 2)}s] [Action]: {name}({args})")
                        _last = now
                        ...
                        if latency:
                            ...
                            if self.verbose:
                                print(f"[tool {tool_sec:.3f}s] [Result]: {part.function_response.response}")
                        else:
                            if self.verbose:
